...
function ReservasLogo() {
	...
	useEffect(() => {
	  ...
	  // Limpiar elementos de ejecuciones anteriores (HMR)
	  Array.from(clipGroup.querySelectorAll('.rm')).forEach(el => el.remove());
	  Array.from(wrap.querySelectorAll('.particle')).forEach(el => el.remove());
	  ...
	  const SC = 0.6935;
	  const rays = [
		[[-137.3, -191.3], [-98.5, -253.4]], [[-190.9, -55.2], [-168.3, -124.9]],
		[[-210.0, 89.8], [-204.9, 16.7]], [[-193.4, 235.1], [-206.1, 162.9]],
		[[-142.2, 372.0], [-172.0, 305.1]], [[-59.4, 492.6], [-104.5, 434.8]],
		[[50.2, 589.5], [-7.6, 544.4]], [[179.9, 657.0], [113.0, 627.2]],
		[[322.1, 691.1], [249.9, 678.4]], [[468.3, 689.9], [395.2, 695.0]],
		[[609.9, 653.3], [540.2, 675.9]], [[738.4, 583.5], [676.3, 622.3]],
		[[846.2, 484.7], [795.4, 537.4]], [[927.0, 362.7], [890.3, 426.2]],
	  ];
  
	  function tp(p: number[]) { return [SNX + (p[0] - SOX) * SC, SNY + (p[1] - SOY) * SC]; }
  
	  rays.forEach(([p1, p2]) => {
		const np1 = tp(p1), np2 = tp(p2);
		const el = document.createElementNS(NS, 'polygon');
		el.setAttribute('class', 'rm');
		el.setAttribute('points', `${SNX},${SNY} ${np1[0].toFixed(1)},${np1[1].toFixed(1)} ${np2[0].toFixed(1)},${np2[1].toFixed(1)}`);
	  ...
	  for (let i = 0; i < particleCount; i++) {
		...
		p.style.cssText = `width:${s}px;height:${s}px;left:${Math.random() * wrapWidth}px;bottom:${Math.random() * (wrapHeight * 0.25)}px;background:${c};animation-duration:${Math.random() * 4 + 3}s;animation-delay:${Math.random() * 6}s;will-change:transform,opacity;transform:translateZ(0);`;
		wrap.appendChild(p);
		createdParticles.push(p);
	  }
  
	  return () => {
		createdRays.forEach(el => el.remove());
	...
	return (
	  ...
	  >
		<svg width="100%" height="auto" viewBox="0 0 600 500" xmlns="http://www.w3.org/2000/svg" overflow="visible">
			...
			<text className="title" x="300" y="370" textAnchor="middle" fontFamily="'Playfair Display', Georgia, serif" fontSize="84" fontWeight="900" fill="#FF00FF">MARBELLA</text>
			<text className="sub" x="300" y="408" textAnchor="middle" fontFamily="'Montserrat', Verdana, sans-serif" fontSize="25" fontWeight="300" letterSpacing="7" fill="#FFFFFF">LET'S PARTY</text>
		</svg>
	  </div>
	);
  }
  ...
  return (
    <>
      <div className="app" ref={rootRef}>
        <header>
          <div className="logo-tag"><span>Reservas Exclusivas</span></div>
          <div className="w-full max-w-lg mx-auto">
            <ReservasLogo />
          </div>
          <div className="subtitle"><span>Selecciona tu espacio - Vive la experiencia</span></div>
        </header>
