...
export default function Page() {
  ...
  const seatById = useMemo(() => new Map(SEATS.map((seat) => [seat.id, seat])), []);
  const selectedArray = useMemo(() => Array.from(selected), [selected]);
  const total = useMemo(() => selectedArray.reduce((sum, id) => sum + (seatById.get(id)?.price ?? 0), 0), [selectedArray, seatById]);
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      tl.fromTo('.decor-stage-name', { autoAlpha: 0, y: -8 }, { autoAlpha: 1, y: 0, duration: 0.28 })
        .fromTo(
      ...
      screensTl
        .to('.decor-ceiling.c1', {
          ...
          boxShadow: 'inset 0 0 24px rgba(0, 0, 0, 0.25), 0 0 22px rgba(255, 198, 92, 0.45)',
          ease: 'power2.out',
        })
        .to('.decor-ceiling.c1', {
          duration: 0.3,
        ...
        .to('.decor-ceiling.c2', {
          ...
          boxShadow: 'inset 0 0 26px rgba(0, 0, 0, 0.2), 0 0 28px rgba(255, 214, 120, 0.5)',
          ease: 'power2.out',
        }, '-=0.08')
        .to('.decor-ceiling.c2', {
          duration: 0.32,
        ...
        .to('.decor-stage-name', {
          duration: 0.28,
          textShadow: '0 2px 8px rgba(0, 0, 0, 0.6), 0 0 16px rgba(255, 226, 160, 0.35)',
          ease: 'sine.inOut',
          yoyo: true,
          repeat: 1,
        }, '-=0.12');
    ...
    <>
      <section className="home-hero">
        ...
        <div className="hero-content">
          <div className="hero-badge">Marbella Lounge</div>
          <h1 className="hero-title">Noches de viernes, sabado y domingo</h1>
          <p className="hero-text">
            Vive el ambiente de Marbella con shows en vivo, DJs invitados y la mejor energia del fin de semana.
            Atencion especial en zona lounge y experiencia enfocada en musica.
          </p>

          <div className="hero-days">
            <div className="hero-day-card">
              <strong>Viernes</strong>
              <span>Live show y apertura del fin de semana</span>
            </div>
            <div className="hero-day-card">
              <strong>Sabado</strong>
              <span>Noche central con artistas y pista activa</span>
            </div>
            <div className="hero-day-card">
              <strong>Domingo</strong>
              <span>Cierre con musica y ambiente exclusivo</span>
            </div>
          </div>

          <a href="#reservas-section" className="hero-btn">
            RESERVAR AHORA
          </a>
        </div>
      </section>

      <div id="reservas-section" className="app" ref={rootRef}>
        <header>
        ...
        <div className="legend">
          <div className="legend-item">
            <div className="lb lb-vip" /> VIP Lounge
          </div>
          <div className="legend-item">
            <div className="lb lb-lounge" /> Lounge
          </div>
          <div className="legend-item">
            <div className="lb lb-mesa" /> Mesa
          </div>
          <div className="legend-item">
            <div className="lb lb-m" /> Mesa M
          </div>
          <div className="legend-item">
            <div className="lb lb-sel" /> Seleccionado
          </div>
        </div>
        ...
        <div className="main">
          ...
          <div className="panel">
            ...
            <div className="info-box">
              <strong>VIP Lounge</strong> - Bs. 150 - Consumo min. incluido
              <br />
              <strong>Lounge</strong> - Bs. 80 - Sofa privado
              <br />
              <strong>Mesa</strong> - Bs. 35-40 - Mesa estandar
              <br />
      ...
      <div className={`overlay ${isModalOpen ? 'active' : ''}`} onClick={(e: MouseEvent<HTMLDivElement>) => e.target === e.currentTarget && setIsModalOpen(false)}>
        <div className="modal">
          ...
          <div className="modal-det">
            <div>
              ...
              <span>{modalDetails?.nombre ?? '-'}</span>
            </div>
            <div>
              <strong>Telefono:</strong>
              <span>{modalDetails?.telefono ?? '-'}</span>
            </div>
            <div>
              <strong>Fecha:</strong>
              <span>{modalDetails?.fecha ?? '-'}</span>
            </div>
            <div>
              <strong>Personas:</strong>
              <span>{modalDetails?.personas ?? '-'}</span>
            </div>
            <div>
              <strong>Espacios:</strong>
              <span style={{ fontSize: '9px' }}>{modalDetails?.espacios ?? '-'}</span>
            </div>
            <div>
              <strong>Total:</strong>
              <span>Bs. {modalDetails?.total ?? 0}</span>
            </div>
          </div>
          <button className="btn-ok" onClick={() => setIsModalOpen(false)}>
            ACEPTAR
          </button>
        </div>
      </div>
